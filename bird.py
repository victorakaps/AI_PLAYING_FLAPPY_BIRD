...
class Bird():
    ...
    def get_inpputs(self, pipes):
        closet = DISPLAY_W * 2
        bottom_y = 0
        for p in pipes:
            if p.pipe_type == PIPE_UPPER and p.rect.right < closet and p.rect.right > self.rect.left:
                closet = p.rect.right
                bottom_y = p.rect.bottom

        horizontal_distance = closet - self.rect.centerx
        vertical_distance = (self.rect.centery) - \
            (bottom_y + PIPE_GAP_SIZE / 2)
        inputs = [
            ((horizontal_distance / DISPLAY_W) * 0.99) + 0.01,
            (((vertical_distance + Y_SHIFT) / NORMALIZER) * 0.99)
        ]
        return inputs
